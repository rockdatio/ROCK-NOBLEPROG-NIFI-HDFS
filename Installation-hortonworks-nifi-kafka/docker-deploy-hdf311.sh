...
# THIS ARGUMENTS WORKS ONLY IN LINUX.
pathMountSource=$(pwd)
...
docker run --privileged --name $name -h $hostname --network=cda --network-alias=$hostname -v $mountSource:$mountSink -d "$registry/$name:$version"
